...
def criar_pagamento(descricao, valor, data, metodo, id_transacao=None):
    ...
    if not descricao or not descricao.strip():
        log.warning("CREATE falhou | motivo=descricao_vazia")
    ...
    if not _validar_data(data):
        log.warning("CREATE falhou | motivo=data_invalida | data=%s", data)
        return 400, "Data inválida. Utilize o formato YYYY-MM-DD."
    if not _validar_metodo(metodo):
        log.warning("CREATE falhou | motivo=metodo_invalido | metodo=%s", metodo)
...
def listar_pagamentos(metodo=None):
    ...
    if not pagamentos:
        log.warning("READ listar_pagamentos | motivo=sem_registos")
        return 404, "Não existem pagamentos registados."

    if metodo is not None:
        if not _validar_metodo(metodo):
            log.warning("READ listar_pagamentos | motivo=metodo_invalido | metodo=%s", metodo)
            return 400, f"Método inválido. Opções: {', '.join(METODOS_PAG)}."
        filtrado = {k: v for k, v in pagamentos.items() if v["metodo"] == metodo}
        if not filtrado:
            log.warning("READ listar_pagamentos | motivo=sem_resultados | metodo=%s", metodo)
...
def consultar_pagamento(id_pagamento):
    ...
    if id_pagamento not in pagamentos:
        log.warning("READ consultar_pagamento | motivo=nao_encontrado | id=%s", id_pagamento)
...
def atualizar_pagamento(id_pagamento, descricao=None, valor=None, data=None, metodo=None, id_transacao=None):
    ...
    if id_pagamento not in pagamentos:
        log.warning("UPDATE pagamento | motivo=nao_encontrado | id=%s", id_pagamento)
        return 404, "Pagamento não encontrado."

    if descricao is not None:
        if not descricao.strip():
            log.warning("UPDATE pagamento | motivo=descricao_vazia | id=%s", id_pagamento)
            return 400, "Descrição não pode ser vazia."
        pagamentos[id_pagamento]["descricao"] = descricao.strip()

    if valor is not None:
        if not _validar_valor(valor):
            log.warning("UPDATE pagamento | motivo=valor_invalido | id=%s | valor=%s", id_pagamento, valor)
            return 400, "Valor inválido. Deve ser um número positivo."
        pagamentos[id_pagamento]["valor"] = float(valor)

    if data is not None:
        if not _validar_data(data):
            log.warning("UPDATE pagamento | motivo=data_invalida | id=%s | data=%s", id_pagamento, data)
            return 400, "Data inválida. Utilize o formato YYYY-MM-DD."
        pagamentos[id_pagamento]["data"] = data

    if metodo is not None:
        if not _validar_metodo(metodo):
            log.warning("UPDATE pagamento | motivo=metodo_invalido | id=%s | metodo=%s", id_pagamento, metodo)
            return 400, f"Método inválido. Opções: {', '.join(METODOS_PAG)}."
...
def remover_pagamento(id_pagamento):
    ...
    if id_pagamento not in pagamentos:
        log.warning("DELETE pagamento | motivo=nao_encontrado | id=%s", id_pagamento)
        return 404, "Pagamento não encontrado."
